...
MockModule = {
	...
	createMockEntitiesInLayer: function( layer ){			
		...
		if (layer.objects) {
			...
			for (var i = 0; i < layer.objects.length; i++) {
				...
				var mock = {head:"2D," + this._renderMethod  + "," + layer.name + "," + obj.type};
				mock.x = obj.x;
				mock.y = obj.y;
				mock.width  = obj.width;
				mock.height = obj.height;
				mock.type = obj.type;
				mock.roaming = true;
				mock.properties = obj.properties;
				entities.push(mock);
			}
		} else {
			for(var i = 0; i < indexes.length; i++){	
				
				if( layer.data && layer.data[indexes[i]] == 0 ){
					entities.push(0);
				} else if (layer.data) {											
	...
	*/
	...
	createMockEntity:function( layer, dataIndex){			
		...
		var mock = {head:"2D," + this._renderMethod  + (layer.data ? ",Tile" + layer.data[dataIndex] : "") + "," + layer.name};	
